...
gray = [x.flatten() for x in resized]

Tgray = [x.flatten() for x in Tresized]

x = tf.placeholder(dtype=tf.float32, shape=[None, 2700])
y = tf.placeholder(dtype=tf.int32, shape=[None, 62])


def net(data):
    hidden_l1 = {"w": tf.Variable(tf.random_normal([2700, 900])), "b": tf.Variable(tf.random_normal([900]))}
